...
public class Quarry {
    ...
    public void doTick() {
        ...
        if (tier == 0) {
            WorldFunctions.queueBlock(block, Material.IRON_BLOCK.getId(), (byte) 0);
        }
        if (tier == 1) {
            WorldFunctions.queueBlock(block, Material.GOLD_BLOCK.getId(), (byte) 0);
        }
        if (tier == 2) {
            WorldFunctions.queueBlock(block, Material.OBSIDIAN.getId(), (byte) 0);
        }
        if (!cantick) {
            return;
        }
